...
class AesKeyWrapKekContext(IKekContext):
    ...
    """
    def __init__(self, logger, kek) -> None:
        self._logger = Preconditions.check_not_null( logger )
        self._kek_obj = Preconditions.check_not_null( kek )
        self._kek = self._kek_obj.get_key()
    ...
    def wrap_key(self, key_to_wrap: bytes) -> bytes:
        ...
        return self._kek_obj.get_kid() + aes_key_wrap(wrapping_key=self._kek, key_to_wrap=key_to_wrap)
